...
namespace Manatee.Json.Schema
{
    ...
    public class JsonSchema : IJsonSchema, ICanReferenceSchema
    {
        ...
        void ICanReferenceSchema.ResolveReferences(JsonValue root, string documentPath)
        {
            if (ReferenceEquals(Type?.Definition, this)) return;

            root = root ?? ToJson(null);
            (AdditionalItems?.Definition as ICanReferenceSchema)?.ResolveReferences(root, documentPath);
            (Items as ICanReferenceSchema)?.ResolveReferences(root, documentPath);
            (AdditionalProperties?.Definition as ICanReferenceSchema)?.ResolveReferences(root, documentPath);
            if (Definitions != null)
            {
                foreach (var schema in Definitions.Select(d => d.Definition))
                {
                    (schema as ICanReferenceSchema)?.ResolveReferences(root, documentPath);
                }
            }
            if (Properties != null)
            {
            ...
            (Type?.Definition as ICanReferenceSchema)?.ResolveReferences(root, documentPath);
            if (AllOf != null)
            {
            ...
            (Not as ICanReferenceSchema)?.ResolveReferences(root, documentPath);
        }
